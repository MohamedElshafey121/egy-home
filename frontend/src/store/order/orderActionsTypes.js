export const ORDER_CREATE_REQUEST = "ORDER_CREATE_REQUEST";
export const ORDER_CREATE_SUCCESS = "ORDER_CREATE_SUCCESS";
export const ORDER_CREATE_FAIL = "ORDER_CREATE_FAIL";

export const ORDER_MY_LIST_REQUEST = "ORDER_MY_LIST_REQUEST";
export const ORDER_MY_LIST_SUCCESS = "ORDER_MY_LIST_SUCCESS";
export const ORDER_MY_LIST_FAIL = "ORDER_MY_LIST_FAIL";

export const ORDER_LIST_REQUEST = "ORDER_LIST_REQUEST";
export const ORDER_LIST_SUCCESS = "ORDER_LIST_SUCCESS";
export const ORDER_LIST_FAIL = "ORDER_LIST_FAIL";

export const GET_ORDER_REQUEST = "GET_ORDER_REQUEST";
export const GET_ORDER_SUCCESS = "GET_ORDER_SUCCESS";
export const GET_ORDER_FAIL = "GET_ORDER_FAIL";

export const CANCEL_ORDER_REQUEST = "CANCEL_ORDER_REQUEST";
export const CANCEL_ORDER_SUCCESS = "CANCEL_ORDER_SUCCESS";
export const CANCEL_ORDER_FAIL = "CANCEL_ORDER_FAIL";

export const GET_USER_ORDERS_LIST_REQUEST = "GET_USER_ORDERS_LIST_REQUEST";
export const GET_USER_ORDERS_LIST_SUCCESS = "GET_USER_ORDERS_LIST_SUCCESS";
export const GET_USER_ORDERS_LIST_FAIL = "GET_USER_ORDERS_LIST_FAIL";

export const CHECK_TRACK_ORDER_REQUEST = "CHECK_TRACK_ORDER_REQUEST";
export const CHECK_TRACK_ORDER_SUCCESS = "CHECK_TRACK_ORDER_SUCCESS";
export const CHECK_TRACK_ORDER_FAIL = "CHECK_TRACK_ORDER_FAIL";
export const CHECK_TRACK_ORDER_RESET = "CHECK_TRACK_ORDER_RESET";
